...
object Interaction2 extends Interaction2Interface:
  ...
  def availableLayers: Seq[Layer] =
    Array(
      Layer(LayerName.Temperatures, colors, 1975 to 2015),
      Layer(LayerName.Deviations, colorsDiff, 1975 to 2015)
    )
  ...
  def yearSelection(selectedLayer: Signal[Layer], sliderValue: Signal[Year]): Signal[Year] =
    Signal {
      sliderValue() match
        case y if y > selectedLayer().bounds.max => selectedLayer().bounds.max
        case y if y < selectedLayer().bounds.min => selectedLayer().bounds.min
        case _ => sliderValue()
    }
  ...
  def caption(selectedLayer: Signal[Layer], selectedYear: Signal[Year]): Signal[String] =
    Signal {f"${selectedLayer().layerName.id.capitalize} (${selectedYear()})"}
